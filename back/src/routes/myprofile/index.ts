...
import { inputMyprofileJson } from "./types/json-convert-type";
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        fs.writeFileSync(
          path.resolve(
            __dirname,
            `../../../errorlogs/${currentDate.getDate()}_${currentDate.getHours()}_${currentDate.getMinutes()}_${currentDate.getSeconds()}`
          ),
          err.message
        );
      }
      res
        .status(400)
        .json({ message: "バリデーションに引っかからずにupdate出来なかった" });
    }
  }
);
